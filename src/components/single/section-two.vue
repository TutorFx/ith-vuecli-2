    ...
    <v-container class="container-Curso">
      <v-row align="end">
        <v-col cols="" xs="12" md="7" class="mb-5">
          <div
            v-if="$store.state.api.cursoLoaded"
            class="Titulo mb-3 font-weight-light"
          >
            {{
              curso.acf.categoria == 1
                ? "Pós Graduação"
                : curso.acf.categoria == 2 || curso.acf.categoria == 3
                ? "Extensão"
                : curso.acf.categoria == 4
                ? "Internacional"
                : curso.acf.categoria == 5
                ? "Técnico"
                : curso.acf.categoria == 6
                ? "Graduação"
                : "Error 404"
            }}
          </div>
          ...
          <v-card-text
            v-if="$store.state.api.cursoLoaded"
            class="ml-xl-n5 ml-lg-n5"
          >
            <v-chip-group dark>
              <v-chip
                color="#fca311"
                outlined
                v-for="(publico, i) in curso.acf.publico_alvo"
                :key="i"
                >{{ publico.name }}</v-chip
              >
            </v-chip-group>
          ...
          <div
            v-if="$store.state.api.cursoLoaded"
            v-html="curso.acf.descricao"
            class="Desc mt-3 mb-7"
          ></div>
          ...
          <v-row v-if="$store.state.api.cursoLoaded">
            ...
            <v-col
              v-if="curso.acf.carga_horaria_h"
              cols=""
              xs="12"
              sm="12"
              md="6"
            ...
            <v-col
              v-if="curso.acf.carga_horaria_m"
              cols=""
              xs="12"
              sm="12"
              md="6"
            ...
            <div class="container-matricular">
              ...
              <div class="intersection">
                ...
                <div class="intersection-botao">
                  <v-btn
                    depressed
                    :disabled="!curso.acf.id"
                    dark
                    color="#222C35"
                    @click="
                      curso.acf.id != undefined
                        ? storage.carrinho.push(
                            JSON.stringify({
                              img: curso.thumbnail.large,
                              id: curso.acf.id,
                              url: rota,
                            })
                          )
...
export default {
  ...
  methods: {
    ...
    abrirWhatsapp(titulo) {
      window.open(
        `https://api.whatsapp.com/send?phone=556230917079&text=Ol%C3%A1,%20eu%20gostaria%20de%20me%20inscrever%20para%20o%20curso%20de%20${decodeURI(
          titulo
        )}.`
      );
    },
